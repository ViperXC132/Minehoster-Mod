...
public final class MinehosterScreen extends Screen {
    ...
    @Override public void render(DrawContext c,int mouseX,int mouseY,float delta){
        ...
        // Overview
        panel(c,mx,58,rx-mx-14,122);if(s==null){c.drawText(textRenderer,"No servers yet",mx+22,79,TEXT,true);c.drawText(textRenderer,"Enter a name above and create your first server.",mx+22,99,MUTED,false);}else{c.fill(mx+20,77,mx+72,129,0xFF25331E);c.fill(mx+27,84,mx+65,122,0xFF3B7C32);c.drawText(textRenderer,s.name(),mx+88,77,TEXT,true);c.drawText(textRenderer,type(s),mx+88,98,MUTED,false);c.drawText(textRenderer,s.directory().toString(),mx+88,116,MUTED,false);c.drawText(textRenderer,"JAR",mx+88,137,BLUE,true);boolean jar=java.nio.file.Files.exists(s.serverJar());c.drawText(textRenderer,jar?"Installed":"Missing",mx+119,137,jar?GREEN:RED,false);c.drawText(textRenderer,"Process",mx+88,153,MUTED,false);c.drawText(textRenderer,s.isRunning()?"Online":"Offline",mx+135,153,s.isRunning()?GREEN:MUTED,false);} 
        int statX=rx-205;c.fill(statX,70,rx-26,166,PANEL2);c.drawText(textRenderer,"Status",statX+14,82,MUTED,false);c.drawText(textRenderer,s!=null&&s.isRunning()?"ONLINE":"OFFLINE",statX+14,99,s!=null&&s.isRunning()?GREEN:MUTED,true);c.drawText(textRenderer,"JRE",statX+14,122,MUTED,false);c.drawText(textRenderer,"Java 21",statX+55,122,TEXT,false);c.drawText(textRenderer,"Console",statX+14,143,MUTED,false);c.drawText(textRenderer,s==null?"0 lines":s.logs().size()+" lines",statX+62,143,TEXT,false);
        button(c,"▶  Start",mx,188,94,27,GREEN,true,mouseX,mouseY);button(c,"■  Stop",mx+101,188,94,27,RED,false,mouseX,mouseY);button(c,"↻  Restart",mx+202,188,94,27,BLUE,false,mouseX,mouseY);button(c,"↯  Kill",mx+303,188,80,27,RED,false,mouseX,mouseY);
        // Console
        int cy=226,ch=Math.max(150,height-330),cw=consoleW();panel(c,mx,cy,cw,ch);c.drawText(textRenderer,"▣  Console",mx+14,cy+12,TEXT,true);c.drawText(textRenderer,"LIVE",mx+88,cy+12,s!=null&&s.isRunning()?GREEN:MUTED,true);button(c,"Clear",mx+cw-76,cy+7,60,20,MUTED,false,mouseX,mouseY);int top=cy+34,bottom=cy+ch-43;c.fill(mx+10,top,mx+cw-10,bottom,0xFF090C10);if(s!=null){List<String> logs=s.logs();int max=Math.max(1,(bottom-top-8)/10);int start=Math.max(0,logs.size()-max-logOffset),end=Math.min(logs.size(),start+max),ly=top+8;for(int i=start;i<end;i++){String line=logs.get(i);int chars=Math.max(20,(cw-32)/6);if(line.length()>chars)line=line.substring(0,chars-1)+"…";int col=line.contains("ERROR")||line.contains("Exception")?RED:(line.contains("Done")||line.contains("ready")?GREEN:0xFFD1D6DE);c.drawText(textRenderer,line,mx+16,ly,col,false);ly+=10;}}else c.drawText(textRenderer,"No server selected. Create one to see live output.",mx+18,top+18,MUTED,false);int cmdY=cy+ch-34;c.fill(mx+10,cmdY,mx+cw-78,cmdY+27,0xFF0C1015);c.drawText(textRenderer,">",mx+18,cmdY+9,GREEN,true);button(c,"Send",mx+cw-70,cmdY,60,27,GREEN,true,mouseX,mouseY);
        // Controls
        panel(c,rx,188,216,150);c.drawText(textRenderer,"Server Controls",rx+14,202,GREEN,true);button(c,"Settings",rx+12,220,91,31,PURPLE,false,mouseX,mouseY);button(c,"Files",rx+108,220,91,31,BLUE,false,mouseX,mouseY);button(c,"Plugins",rx+12,257,91,31,PURPLE,false,mouseX,mouseY);button(c,"Worlds",rx+108,257,91,31,BLUE,false,mouseX,mouseY);button(c,"Java 21",rx+12,294,91,31,GREEN,false,mouseX,mouseY);button(c,"Downloads",rx+108,294,91,31,BLUE,false,mouseX,mouseY);
        c.fill(rx,346,rx+216,376,PANEL);c.drawText(textRenderer,"Install plugin",rx+12,352,MUTED,false);c.fill(rx+10,369,rx+138,392,0xFF0C1015);button(c,"Install",rx+140,369,66,23,GREEN,true,mouseX,mouseY);
        int iy=386;panel(c,rx,iy,216,126);c.drawText(textRenderer,"Server Info",rx+14,iy+13,TEXT,true);info(c,"Server Name",s==null?"—":s.name(),rx,iy+34);info(c,"Type",s==null?"—":type(s),rx,iy+51);info(c,"Server JAR",s!=null&&java.nio.file.Files.exists(s.serverJar())?"Installed":"Missing",rx,iy+68);info(c,"JRE",JreManager.findJava(MinehosterClient.SERVERS.jres())==null?"Not installed":"Java 21",rx,iy+85);info(c,"Status",s!=null&&s.isRunning()?"Running":"Stopped",rx,iy+102);
        // Server list
        int ly=height-116;panel(c,mx,ly,width-mx-14,96);c.drawText(textRenderer,"Your Servers",mx+14,ly+12,TEXT,true);button(c,"↻ Refresh",width-120,ly+7,90,22,MUTED,false,mouseX,mouseY);button(c,"+ Create Server",width-220,ly+7,92,22,GREEN,true,mouseX,mouseY);var servers=MinehosterClient.SERVERS.all();int row=ly+35,i=0;for(var server:servers){if(server==s)c.fill(mx+8,row-2,width-22,row+21,0xFF16251A);c.drawText(textRenderer,server.isRunning()?"●":"○",mx+16,row+3,server.isRunning()?GREEN:MUTED,false);c.drawText(textRenderer,server.name(),mx+32,row+3,server==s?GREEN:TEXT,true);c.drawText(textRenderer,type(server),mx+150,row+3,MUTED,false);c.drawText(textRenderer,server.isRunning()?"Running":"Stopped",width-158,row+3,server.isRunning()?GREEN:MUTED,false);row+=24;if(++i>=2)break;}if(servers.isEmpty())c.drawText(textRenderer,"No servers found — create one above.",mx+14,row+3,MUTED,false);
    ...
    @Override public boolean mouseClicked(double x,double y,int b){if(b!=0)return super.mouseClicked(x,y,b);int mx=mainX(),rx=rightX();
        ...
        if(x>=mx+188&&x<=mx+280&&y>=40&&y<=67){createServer();return true;}
        ...
        if(y>=188&&y<=215){if(x>=mx&&x<mx+94)start();else if(x>=mx+101&&x<mx+195)stop();else if(x>=mx+202&&x<mx+296){stop();status="Restart requested";}else if(x>=mx+303&&x<mx+383){var t=target();if(t!=null&&t.process()!=null)t.process().destroyForcibly();status="Process killed";}return true;}
        ...
        if(x>=rx+140&&x<=rx+206&&y>=369&&y<=392){installPlugin();return true;}
        int cy=226,ch=Math.max(150,height-330),cw=consoleW();if(y>=cy&&y<=cy+ch){if(x>=mx+cw-76&&y<=cy+30){status="Console clear is not destructive; logs remain available.";return true;}commandBox.setFocused(true);}
